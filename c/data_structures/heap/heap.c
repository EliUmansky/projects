...
#define DVEC_NUM_OF_ELEMENTS (15)
...
enum find_element {NOT_FOUND, FOUND};
...
static size_t GetChildIndex(size_t parent_index, int side);
static size_t FindIndexOfRemoved(heap_t *heap, const void *data, 
								 match_func_t is_matched, int *is_found);
...
void *HeapRemove(heap_t *heap, match_func_t is_matched, const void *data)
{
	...
	index_of_removed = FindIndexOfRemoved(heap, data, is_matched, &is_found);
	if (NOT_FOUND == is_found)
	{
		return NULL;
	}
								 ...
								 match_func_t is_matched, int *is_found)
{
	...
	if (index >= HeapSize(heap) - 1 && 0 == status)
	{
		*is_found = NOT_FOUND;
	}
...
static size_t GetSmallestChildIndex(heap_t *heap, size_t parent_index)
{
	size_t child_l_index = GetChildIndex(parent_index, LEFT);
	size_t child_r_index = GetChildIndex(parent_index, RIGHT);
...
static size_t GetChildIndex(size_t parent_index, int side)
{
	return (LEFT == side) ? (parent_index * 2) + 1 : (parent_index * 2) + 2;
}
...
static void HeapifyUp(heap_t *heap, size_t child_index)
{
	...
	if (FIRST_INDEX_IN_ARR == child_index)
	{
		return;
	}
