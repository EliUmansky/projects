...
/*
static void Merge(slist_node_t *head_dest, slist_node_t *head_src)
{
	slist_node_t *head_src_next = head_src->next;	
	...
	head_dest = MergeNodes(head_dest, head_src);
}

static slist_node_t *MergeNodes(slist_node_t *head_dest, slist_node_t *head_src)
{	
	if (*(int*)head_src->data < *(int*)head_dest->data)
	{
		SlistInsert(head_dest, head_src);
		return head_src;
	}
	else
	{
		return MergeNodes(head_dest, head_src->next);
	}
}
